...
export function AddRateLimiting(action, identifier, calls, perMilliSeconds) {
  return Let(
    ...
    // An access logs page from that index looks like
    // { data: [timestamp1, timestamp2,...]},

    If(
      // Either the page is empty or there are less calls than the limit logged.
      Or(
        IsEmpty(Var('logsPage')),
        LT(Count(Select(['data'], Var('logsPage'))), calls)
      ),
      // In that case, Create a log and return.
      Do(CreateAccessLogEntry(action, identifier), true),
      // Else verify whether the last timestamp of that page is within the
      // 'perMilliSeconds' timewindow (the index is ordered on ts)
      // If it is, you have done too many calls within the same timewindow
