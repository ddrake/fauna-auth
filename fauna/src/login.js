    ...
    Let(
      {
        ...
        // The below (Identify + CreateAccessToken) is equivalent of using
        // the Login() functionality in Fauna except that you have
        // more control. The Login function would abort in case the password
        // is wrong and since you might want to call it from the frontend,
        // it ideally doesn't return an error to the user.
        // Verify whether our login credentials are correct with Identify.
        authenticated: Identify(Var('accountRef'), password)
        // Once identified, an access and refresh in case
        // authentication succeeded.
      },
      {
        token: If(
          Var('authenticated'),
          CreateAccessToken(Var('accountRef'), accessTtlSeconds),
          false
        ),
        // Do not provide the end user with information about existence
        // of the account
