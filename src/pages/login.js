...
const handleLogin = (event, username, password, navigate, sessionContext) => {
  faunaQueries
  ...
  if (!user) {
    return (
      <Form
        title="Login"
        formType="login"
        handleSubmit={(event, username, password) => handleLogin(event, username, password, navigate, sessionContext)}
      ></Form>
    )
  } else {
    return (
